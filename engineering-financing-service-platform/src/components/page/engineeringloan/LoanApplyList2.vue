<template>
    <div>
                    ...
                    <div class="loan-info-box" v-loading="loading">
                        <div class="list" v-for=" (item, index) in tableData" :key="index">
                            <div class="loan-info-box list loan-info">
                                ...
                                <div class="tag-status" v-if="item.step === 2 && item.status != -1">待确认</div>
                                <div class="tag-status" v-if="item.step === 2 && item.status === -1">已拒绝</div>
                                <div class="btn-cancel" v-if="item.step === 2 && item.status != -1" @click="gotoSubmit(item.id, item.applyNo, -1)">拒绝</div>
                                <div class="btn-submit" v-if="item.step === 2 && item.status != -1" @click="gotoSubmit(item.id, item.applyNo, 1)">确认</div>
                            </div>
        ...
        <!-- 审批弹出框 -->
        <el-dialog :visible.sync="submitVisible" center width="500px">
            <el-form ref="form" :model="form" :rules="rules" label-width="170px">
                ...
                <el-form-item label="请输入拒绝原因"  v-show="this.form.result === -1" prop="opinion">
                    <el-input type="textarea" v-model="form.opinion" style="width: 250px"></el-input>
                </el-form-item>
...
<script>
    export default {
        ...
        methods: {
            // 获取数据
            getTableData(){
                this.$axios.get('la/list').then( res => {
                    console.log(res);
            ...
            uploadSuccess1(res,file,files){
                this.form.onsiteFile = res.extend.fileSystem.filePath;
            },
            uploadSuccess2(res,file,files){
                this.form.creditrateFile = res.extend.fileSystem.filePath;
            },
                            ...
                            this.qs.stringify(
                                {
                                    ...
                                    creditrateFile: this.form.creditrateFile,
